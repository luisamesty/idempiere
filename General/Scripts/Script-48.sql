SELECT 
    ...
    -- emp.birthday as fecha_nacimiento, emp.incomedate as fecha_ingreso, emp.egressdate as fecha_retiro,
   ...
   CASE WHEN ( $P{AMN_Contract_ID}  IS NULL OR amc.amn_contract_id= $P{AMN_Contract_ID} ) THEN 1 ELSE 0 END AS imp_contrato,
    ...
    -- AMN PROCESS
    pro.amn_process_id, pro.value as proceso, COALESCE(pro.name,'') as proceso_name,
    -- AMN_PAYROLL_DETAIL
    ...
    LEFT JOIN C_Period per ON (per.c_period_id = tiper.c_period_id AND tiper.amn_process_id IN (Select AMN_process_ID FROM AMN_Process pro WHERE pro.Value = 'NN' OR pro.Value = 'NU'))
    ...
    --LEFT JOIN adempiere.amn_concept_types as prcty ON (prcty.amn_concept_types_id = pydma.amn_concept_types_id)
...
WHERE per.c_period_id >=  $P{C_PeriodIni_ID}  AND per.c_period_id <=  $P{C_PeriodEnd_ID} AND per.c_year_id = $P{C_Year_ID}
	...
	AND CASE WHEN ( $P{AMN_Process_ID}  IS NULL OR pro.amn_process_id= $P{AMN_Process_ID} ) THEN 1=1 ELSE 1=0 END
...
ORDER BY tiper.amn_process_id,tiper.amn_contract_id, emp.value, tiper.amndateini