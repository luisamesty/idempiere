-- FUNCTION: adempiere.invoiceopen(numeric, numeric)
-- MODIFIED TO READ AMERP_Currency_Gain_Loss
-- WHEN AMERP_Currency_Gain_Loss ='N' THEN CALL amf_invoiceopen PLUGIN FUNCTION
-- WHEN AMERP_Currency_Gain_Loss ='Y' OR ANY OTHER VALUE OR NULL THEN CALL Standard invoiceopen
-- 

--DROP FUNCTION IF EXISTS  adempiere.invoiceopen(numeric, numeric);

CREATE OR REPLACE FUNCTION adempiere.invoiceopen(
...
BEGIN
	--	Get Currency
	BEGIN
		SELECT	MAX(C_Currency_ID), SUM(GrandTotal), MAX(MultiplierAP), MAX(Multiplier), MAX(AD_Client_ID)
		INTO	v_Currency_ID, v_TotalOpenAmt, v_MultiplierAP, v_MultiplierCM, v_Client_ID
	...
	-- IF 'N' CALL AMF Function
	 IF (v_AMERP_Currency_Gain_Loss ='N') then
	 	select amf_invoiceopen(p_c_invoice_id,p_c_invoicepayschedule_id)
	 	into v_TotalOpenAmt;
	--	 v_TotalOpenAmt:=4;
...
ALTER FUNCTION adempiere.invoiceopen(numeric, numeric)
    OWNER TO adempiere;
