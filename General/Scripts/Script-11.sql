SELECT * FROM (
 ...
 (
   SELECT DISTINCT
		...
		bps.bpartner, 
		bps.issotrx, 
		CONCAT(bps.vpartner,bps.issotrx) as issotrxBP,
		CONCAT(bps.vpartner,bps.issotrx) as issotrxBP,
		--bps.bprep, 
		--bps.bpgroup, 
		--bps.bpchannel,
...
--	 LEFT JOIN (SELECT DISTINCT ON (AD_Client_ID) C_Currency_ID, AD_Client_ID FROM C_AcctSchema ) as sch ON sch.AD_Client_ID = bps.ad_client_id 
  ...
  AND CASE WHEN ( $P{BothPurchaseSales} = 'B' ) THEN 1 = 1
		...
		ELSE 1=0 END 
  AND CASE 	WHEN ( $P{isShowPayDetail} = 'Y' ) THEN 1=1 
  			WHEN ( $P{isShowPayDetail} = 'N' AND  bps.isPaid ='N' ) THEN 1=1 
  	    	ELSE 1=0 END		
 ) as details ON (1= 0)
WHERE (imp_org= 1) 
   OR ( 
      (org_client= $P{AD_Client_ID} AND org_org= $P{AD_Org_ID}  ) 
   OR (ad_client_id= $P{AD_Client_ID} AND ad_org_id= $P{AD_Org_ID} 
  AND ( details.dateacct <= DATE( $P{DateEnd} )))
  )
ORDER BY  details.bprep,details.bpgroup, details.bpchannel,details.vpartner,  details.issotrx, details.documentno_p, details.ord, details.dateacct, org_inf