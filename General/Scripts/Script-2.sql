SELECT * FROM (
 ...
 (
   SELECT DISTINCT
		...
		bps.Inv_printname, bps.Inv_dateacct, bps.Inv_dateinvoiced, bps.Inv_grandtotal,
		bps.All_payment_id, bps.Pay_documentno, bps.Pay_dateacct, bps.Pay_datetrx, bps.Pay_Currency_ID,
		bps.Pay_printname, bps.Pay_ConversionType_ID, bps.Pay_PayAmt, bps.Pay_datedeposit,
		bps.All_charge_id,	
		COALESCE(bps.Cha_name,'') as cha_name,
		-- BPARTNER
		...
		bps.documentno, bps.documentnoa, bps.documentno_p, bps.document, bps.reference,
  	    ...
  	    bps.c_invoice_id, bps.c_payment_id,
		...
		CASE WHEN ( bps.ord= 1 OR bps.ord= 2  ) AND bps.rn = 1 THEN
		  	    CASE WHEN bps.issotrx = 'N' THEN -1*currencyConvertinvoice(bps.c_invoice_id, $P{C_Currency_ID}, bps.source_grandtotal,bps.dateacct)  
		  	    	WHEN bps.issotrx = 'Y' THEN currencyConvertinvoice(bps.c_invoice_id, $P{C_Currency_ID}, bps.source_grandtotal,bps.dateacct) 
		  	    	ELSE currencyConvertinvoice(bps.c_invoice_id, $P{C_Currency_ID}, bps.source_grandtotal,bps.dateacct)  
		  	   		END 
			WHEN ( bps.ord= 11 OR bps.ord= 12  ) AND bps.rn = 1 THEN
		  	    CASE WHEN bps.issotrx = 'N' THEN -1*currencyConvertpayment(bps.c_payment_id, $P{C_Currency_ID}, bps.source_grandtotal,bps.dateacct)  
		  	    	WHEN bps.issotrx = 'Y' THEN currencyConvertpayment(bps.c_payment_id, $P{C_Currency_ID}, bps.source_grandtotal,bps.dateacct) 
		  	    	ELSE currencyConvertpayment(bps.c_payment_id, $P{C_Currency_ID}, bps.source_grandtotal,bps.dateacct)  
		  	   		END 
			WHEN ( bps.ord= 21  ) AND bps.rn = 1 THEN
		...
		CASE WHEN bps.signdbcr ='DB' AND bps.rn = 1 THEN	
			CASE WHEN ( bps.ord= 1 OR bps.ord= 2  ) THEN 
				currencyConvertinvoice(bps.c_invoice_id, $P{C_Currency_ID}, abs(amf_invoiceopentodate(bps.c_invoice_id,bps.C_InvoicePaySchedule_ID, $P{DateEnd})), bps.dateacct)
		  		WHEN ( bps.ord= 11 OR bps.ord= 12  ) THEN
		  		currencyConvertpayment(bps.c_payment_id, $P{C_Currency_ID}, abs(amf_paymentAvailable(bps.c_payment_id, $P{DateEnd})),bps.dateacct)
		  		WHEN ( bps.ord= 21  ) THEN
		...
		-- DR_USD (DEB -DB USD)
		CASE WHEN bps.signdbcr ='DB' AND bps.rn = 1 AND $P{isShowUSD} = 'Y' AND (curr3.c_currency_id != $P{C_Currency_ID}) THEN	
			CASE WHEN ( bps.ord= 1 OR bps.ord= 2  )  THEN 
				currencyConvertinvoice(bps.c_invoice_id, curr3.c_currency_id, abs(amf_invoiceopentodate(bps.c_invoice_id,bps.C_InvoicePaySchedule_ID, $P{DateEnd})), bps.dateacct)
		  		WHEN ( bps.ord= 11 OR bps.ord= 12  ) THEN
		  		currencyConvertpayment(bps.c_payment_id, curr3.c_currency_id, abs(amf_paymentAvailable(bps.c_payment_id, $P{DateEnd})),bps.dateacct)
		  		WHEN ( bps.ord= 21  ) THEN
		...
		-- CR_USD (CRE - CR USD)
		CASE WHEN bps.signdbcr ='CR' AND bps.rn = 1 AND $P{isShowUSD} = 'Y' AND (curr3.c_currency_id != $P{C_Currency_ID}) THEN
			CASE WHEN ( bps.ord= 1 OR bps.ord= 2  ) THEN 
				currencyConvertinvoice(bps.c_invoice_id, curr3.c_currency_id, abs(amf_invoiceopentodate(bps.c_invoice_id,bps.C_InvoicePaySchedule_ID, $P{DateEnd})), bps.dateacct)
		  		WHEN ( bps.ord= 11 OR bps.ord= 12  ) THEN 
		  		currencyConvertpayment(bps.c_payment_id, curr3.c_currency_id, abs(amf_paymentAvailable(bps.c_payment_id, $P{DateEnd})),bps.dateacct)
		...
		CASE WHEN $P{isSummary} != 'Y'  THEN	
			CASE WHEN (  bps.all_ord= 3 OR bps.all_ord= 4 OR bps.all_ord= 5 OR bps.all_ord= 13  OR bps.all_ord= 14 ) OR ((bps.all_ord= 11  OR bps.all_ord= 11) AND bps.all_charge_id IS NOT NULL)THEN 
...
WHERE  (imp_org= 1) 
   ...
   -- FOR TEST ONLY
   --and details.ord in (11,12,13)
   --and details.C_Payment_ID in (1092064, 1092063, 1092062)
   and details.C_invoice_id in (1049070)
   -- FOR TEST ONLY
