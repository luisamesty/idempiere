SELECT * FROM (
 ...
 (
   SELECT DISTINCT
		...
		CASE WHEN bps.signdbcr ='DB' AND bps.rn = 1 THEN	
			CASE WHEN ( bps.ord= 1 OR bps.ord= 2  ) THEN 
				currencyConvertinvoice(bps.c_invoice_id, $P{C_Currency_ID}, abs(amf_invoiceopentodate(bps.document_id,bps.C_InvoicePaySchedule_ID, $P{DateEnd})), bps.dateacct)
		  		WHEN ( bps.ord= 11 OR bps.ord= 12  ) THEN
		  		currencyConvertpayment(bps.c_payment_id, $P{C_Currency_ID}, abs(amf_paymentAvailable(bps.document_id, $P{DateEnd})),bps.dateacct)
		  		WHEN ( bps.ord= 21  ) THEN
		...
		CASE WHEN bps.signdbcr ='DB' AND bps.rn = 1 THEN	
			CASE WHEN ( bps.ord= 1 OR bps.ord= 2  ) AND (curr3.c_currency_id != $P{C_Currency_ID}) THEN 
				currencyConvertinvoice(bps.c_invoice_id, curr3.c_currency_id, abs(amf_invoiceopentodate(bps.document_id,bps.C_InvoicePaySchedule_ID, $P{DateEnd})), bps.dateacct)
		  		WHEN ( bps.ord= 11 OR bps.ord= 12  ) THEN
		  		currencyConvertpayment(bps.c_payment_id, curr3.c_currency_id, abs(amf_paymentAvailable(bps.document_id, $P{DateEnd})),bps.dateacct)
		  		WHEN ( bps.ord= 21  ) THEN
