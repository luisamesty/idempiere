SELECT * FROM (
 ...
 (
   SELECT DISTINCT
	 ...
	 LEFT JOIN ( 
		SELECT  payp1.C_Payment_ID, payp1.IsOverrideCurrencyRate, 
		amf_currencyratepayment (payp1.C_Payment_ID, $P{C_Currency_ID}) as MultiplyRate
		FROM C_Payment payp1
	 ) as conv_pay ON conv_pay.C_Payment_ID = bps.c_payment_id 
	 LEFT JOIN ( 
		SELECT C_invoice_ID, invc1.IsOverrideCurrencyRate,
		amf_currencyrateinvoice (invc1.C_Invoice_ID, $P{C_Currency_ID}) as MultiplyRate
		FROM C_invoice invc1
	 ) as conv_inv ON conv_inv.C_Invoice_ID = bps.c_invoice_id 
