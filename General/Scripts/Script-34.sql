SELECT   
paydet.ad_client_id,   paydet.ad_org_id,  paydet.workforce,  paydet.c_period_id,  
paydet.amn_employee_id,  paydet.value_emp,  paydet.empleado,  
...
FROM (  	
	...
	pyr_d.amountdeducted, pyr_d.amountcalculated, pyr.c_currency_id  	
	...
	AND c_prd.C_Period_ID = 1000094 ) as paydet  
LEFT JOIN (  	
	SELECT   	
	amn_employee_id,  	
	...
	FROM (  		
		...
		LEFT JOIN amn_concept_types_proc as ctp ON (ctp.amn_concept_types_proc_id= pyr_d.amn_concept_types_proc_id)  		
		LEFT JOIN amn_concept_types	as cty ON ((cty.amn_concept_types_id= ctp.amn_concept_types_id))  		
		LEFT JOIN amn_process as prc ON (prc.amn_process_id= ctp.amn_process_id)  		INNER JOIN amn_employee as emp ON (emp.amn_employee_id= pyr.amn_employee_id)  		LEFT JOIN amn_jobtitle as jtt ON (pyr.amn_jobtitle_id= jtt.amn_jobtitle_id)  		LEFT JOIN amn_period as prd ON (prd.amn_period_id= pyr.amn_period_id)  		LEFT JOIN c_period as c_prd ON (c_prd.c_period_id= prd.c_period_id)  		WHERE prc.value ='NP' AND cty.optmode != 'R'        AND pyr.AD_Client_ID = 1000004       AND pyr.AD_Org_ID = 1000004 		AND c_prd.C_Period_ID = 1000094 	) as paydet  	GROUP BY amn_employee_id  ) as paydone ON (paydone.amn_employee_id = paydet.amn_employee_id )  GROUP BY ad_client_id, ad_org_id, workforce, c_period_id, paydet.amn_employee_id, value_emp,empleado, paydone.paydone, paydet.c_currency_id  ORDER BY paydet.workforce, paydet.value_emp  [121]
